...
def set_strict_null(path):
    ...
    try:
        ...
        print(f"Applying strict NULL to {os.path.basename(path)}...", end="", flush=True)
        gdfs = {}
        for l in layers:
        ...
        b_gdf["平均標高"] = np.nan
        b_gdf["平均標高"] = b_gdf["平均標高"].astype("float64")
        gdfs["ishikawa_zukaku_2500_boundary"] = b_gdf
        
        tmp = path + ".strict_null.gpkg"
...
def main():
    ...
    if os.path.exists(g_dir):
        for f in os.listdir(g_dir):
            if f.endswith(".gpkg"):
                set_strict_null(os.path.join(g_dir, f))
                
    set_strict_null(os.path.join(base, "ishikawa_forest_mesh_2500.gpkg"))
